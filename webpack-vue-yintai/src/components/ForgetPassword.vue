  ...
  <div class="wrap">
    ...
    <div class="header">
      <div class="header-left">
        <router-link to="/Login"><img class="login-header-left" src="../../src/assets/luhanran/loginleft.png" alt=""></router-link>
      </div>
      <div class="header-center">
        <span class="login-header-center">忘记密码</span>
      </div>
    ...
    <div class="regi-content">
      <div class="warm-tip-wrap">
        请输入手机号进行验证和重置密码
      </div>

      <div class="field">
        <input @input="changedColorOne" id="cellPhone" ref="isPhoneNumber" type="text" value="" placeholder="请输入手机号" maxlength="11">
      </div>
      <div class="field">
        <input @input="changedColorTwo" ref="isSure" id="validateCode" type="text" value="" placeholder="请输入验证码" maxlength="11">
      ...
      <div class="field">
        <input @input="changedColor" ref="isPassword" id="password" type="text" placeholder="请设置银泰护照密码" maxlength="12">
      </div>

      <p class="useExplain">请设置密码（6到12位，英文+数字）</p>
      <p class="useExplainre">如果您已关联银泰网和银泰护照账号，重置密码将同时重置两个账号的密码。</p>
      <p class="useExplainre">温馨提示：邮箱找回密码功能已下线，邮箱注册用户请通过已绑定手机号找回密码，如果未绑定手机，请联系客服找回400-119-1111</p>
      <div class="last-button">
        <button id="submitButton" @click="isclick" ref="isDisabled" class="forbid" type="button" disabled="disabled">重置密码</button>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      changedColor: function () {
        ...
        if ((/^1[34578]\d{9}$/.test(PhoneNumber)) && (sure !== '') && (Password !== '')) {
...
//          window.location.href = 'http://localhost:8080/UserCenter'
        if (!(reg.test(this.$refs.isPassword.value))) {
